...
use App\Form\Cms\CmsArticleVolunteerDiariesType;
...
/**
 * @Route("/core/cms/volunteerdiaries", name="cms_volunteer_diaries_")
 * @IsGranted("ROLE_SYS_CONTENT_USER")
 */
class CmsArticleVolunteerDiariesController extends AbstractController
{
    /**
    ...
    {
        return $this->render('cms/cms_article_volunteer_diaries/index.html.twig', [
            'cms_articles' => $cmsArticleRepository->findBy(['mstArticleCategory' => 3]),
            'path_index' => 'cms_volunteer_diaries_index',
            'path_add' => 'cms_volunteer_diaries_add',
            'path_edit' => 'cms_volunteer_diaries_edit',
            'path_comment' => 'cms_volunteer_diaries_comment_index',
            'path_show' => 'cms_volunteer_diaries_show',
            'label_title' => 'label.volunteer_diaries',
        ]);
    }
    ...
    public function new(Request $request, CmsArticleRepository $cmsArticleRepository, CommonHelper $commonHelper, FileUploaderHelper $fileUploaderHelper): Response
    {
        ...
        $sequenceNo = $cmsArticleRepository->findOneBySeqNo(3);
        $cmsArticle->setSequenceNo(($sequenceNo[1] + 1));
        $form = $this->createForm(CmsArticleVolunteerDiariesType::class, $cmsArticle, $option);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            */
            ...
            if ($introMediaType == 'image') {
                ...
                $articleIntroTabletFile = $form['articleIntroTabletImage']->getData();
                $articleIntroMobileFile = $form['articleIntroMobileImage']->getData();
...
//                $cmsArticleContent->setMediaType('image');
            ...
            return $this->redirectToRoute('cms_volunteer_diaries_index');
        }
        return $this->render('cms/cms_article_volunteer_diaries/form.html.twig', [
                'cms_article' => $cmsArticle,
                'form' => $form->createView(),
                'index_path' => 'cms_volunteer_diaries_index',
                'label_title' => 'label.volunteer_diaries',
    ...
    {
        return $this->render('cms/cms_article_volunteer_diaries/show.html.twig', [
            'data' => $cmsArticle,
            'index_path' => 'cms_volunteer_diaries_delete',
            'label_button' => 'label.delete',
            'path_index' => 'cms_volunteer_diaries_index',
            'path_edit' => 'cms_volunteer_diaries_edit',
            'path_delete' => 'cms_volunteer_diaries_delete',
        ]);
    }
    ...
    public function edit(Request $request, CmsArticle $cmsArticle, ArticleUpload $articleUpload, CommonHelper $commonHelper, FileUploaderHelper $fileUploaderHelper): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            */
            ...
            // Set up Intro media
            $introMediaType = $form['introMediaType']->getData();
            if(!empty($_POST['cms_article_volunteer_diaries']['removeIntroImage'])) {
                ...
                if ($introMediaType == 'image') {
                    ...
                    if ($articleIntroDesktopFile) {
                        ...
                        // remove old file
                        $fileUploaderHelper->removeFile($articleIntroFileOldDesktop);

                        // upload new file
                        $uniqueFilename = $form['articleIntroImageSetName']->getData().'-'.$uid;

                        $newFilename = $fileUploaderHelper->uploadPublicFile($articleIntroDesktopFile, $uniqueFilename, $articleIntroFileOldDesktop);
                        $cmsArticle->setArticleIntroDesktopImage($newFilename);
                        $cmsArticle->setArticleIntroImageSetName($form->get('articleIntroImageSetName')->getData());
                        $cmsArticle->setArticleIntroDesktopImagePath($this->getParameter('public_file_folder'));
                    }
                    if ($articleIntroTabletFile) {
                        ...
                        // remove old file
                        $fileUploaderHelper->removeFile($articleIntroFileOldTablet);

                        $uniqueFilenameTablet = $articleIntroTabletFileSetName.'-'.$uid;

                        // upload new file
                        $newFilename = $fileUploaderHelper->uploadPublicFile($articleIntroTabletFile, $uniqueFilenameTablet, $articleIntroFileOldTablet);
                        $cmsArticle->setArticleIntroTabletImage($newFilename);
                        $cmsArticle->setArticleIntroTabletImagePath($this->getParameter('public_file_folder'));
                    }
                    if ($articleIntroMobileFile) {
                        ...
                        // remove old file
                        $fileUploaderHelper->removeFile($articleIntroFileOldMobile);

                        $uniqueFilenameMobile = $articleIntroMobileFileSetName.'-'.$uid;

                        // upload new file
                        $newFilename = $fileUploaderHelper->uploadPublicFile($articleIntroMobileFile, $uniqueFilenameMobile, $articleIntroFileOldMobile);
                        $cmsArticle->setArticleIntroMobileImage($newFilename);
                        $cmsArticle->setArticleIntroMobileImagePath($this->getParameter('public_file_folder'));
                    }
                }
