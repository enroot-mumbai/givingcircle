$(document).ready(function () {
    ...
    $('.input-images-3').imageUploader({
        ...
        maxSize: 5248000,
        imagesInputName: 'broadCastImage',
    });
    ...
    $("#sendReminderToMember").html('Send Broadcast Update');
    ...
    $("#sendReminderToParticipant").click(function (){
        var countCheckedCheckboxes = $( ".checkBoxNameParticipant" ).filter(':checked').length;
        if(countCheckedCheckboxes == 0) {
            alert('Please select at least one participant to send broadcast message to')
            return false;
        }
        if ($.trim($("#textBroadCastMessage").val()) == '') {
            alert('Please enter valid message for broadcasting')
        ...
        $("#hdnBroadCastMembers").val(selected.join(','));
        if ($( ".checkBoxNameParticipant" ).length == selected.length) {
            $("#hdnSentTo").val('All');
        } else {
            $("#hdnSentTo").val('Individual');
        }
