$(document).ready(function () {
    ...
    $(document).on('click', "[name='btnAcceptRequest']", function (e){
        var data = {};
        data['requestid'] =  $(this).attr('requestid');
        data['strStatusName'] =  'Activated';
        rejectAcceptPayment(data);
    });

    $(document).on('click', "[name='btnRejectRequest']", function (e){
        var data = {};
        data['requestid'] =  $(this).attr('requestid');
        data['strStatusName'] =  'Rejected';
        rejectAcceptPayment(data);
    });

});
function rejectAcceptPayment(data) {
    $.ajax({
        url: updateOfflinePaymentStatusURL,
        data: data,
        type: "POST",
        dataType: "JSON",
        success: function (data) {
            if(data.status == 1) {
                $.ajax({
                    url: ajaxCrowdfundingDonationURL,
                    data: data,
                    type: "POST",
                    dataType: "HTML",
                    success: function (data) {
                        var product = $("#tblDonationInformation");
                        product.html(data);
                    }
                });
            }
