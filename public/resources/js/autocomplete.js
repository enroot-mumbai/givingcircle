...
(function($, undefined)
{
	...
	p.filter = function(list, query)
	{
		...
		list.forEach(function(w){
			...
			if(self.itemContains(w.name, query)) {
				result.push(w.name+'|'+w.id);
	...
	{
		var niddle_data = item.split("|")[0];
		if(this.itemToString(niddle_data).toLowerCase().indexOf(needle.toLowerCase()) >= 0) {
	...
	p.onSetFormData = function(e, data)
	{
		...
		self.hiddenInput().val(data.toString().toLowerCase());
	};
...
(function($)
{
	...
	var p = TextExtTags.prototype,
		 ...
		 *
		 * @name html.tags
		 * @default '<div class="text-tag"><div class="text-button"><span class="text-label"/><a class="btn-remove"/></div></div>'
		 * @author agorbatchev
		 * @date 2011/08/19
		...
		DEFAULT_OPTS = {
			...
			html : {
				...
				tag  : '<div class="text-tag"><div class="text-button"><span class="text-label"></span><a class="btn-remove">X<a></div></div>'
			}
		}
		;
	...
	p.renderTag = function(tag)
	{
		...
		var split_tags = tag.split('|');
		var formated_tag = split_tags[0];
		node.find('.text-label').text(self.itemManager().itemToString(formated_tag));
		
		if(tag.match(/activity/ig)){
			split_tags = split_tags[1].split('-');
			tag = split_tags[1]+'|activity';
			node.data(CSS_TAG, tag);
		} else if(tag.match(/cruise/ig)) {
			split_tags = split_tags[1].split('-');
			tag = split_tags[1]+'|cruise';
			node.data(CSS_TAG, tag);
		} else {
			node.data(CSS_TAG, tag);
		}
...
(function($)
{
	...
	var p = TextExtAutocomplete.prototype,
		...
		CSS_LABEL          = 'text-label',
		CSS_DOT_LABEL      = CSS_DOT + CSS_LABEL,
		CSS_DOT_SUGGESTION_VALUETYPE = CSS_DOT_SUGGESTION + ' span.valuetype',
	...
	p.onSetSuggestions = function(e, data)
	{
		...
		for(var i = 0; i < datalength; i++){
			dataresultvalue.push(data.result[i]['name']);
			dataresultkey.push(data.result[i]['id']);
		}

		var self        = this,
			suggestions = self._suggestions = dataresultvalue,
			suggestions2 = self._suggestions2 = dataresultkey
			;

		if(data.showHideDropdown !== false) {
			self.trigger(suggestions === null || suggestions.length === 0 ? EVENT_HIDE_DROPDOWN : EVENT_SHOW_DROPDOWN);
			//self.trigger(suggestions2 === null || suggestions2.length === 0 ? EVENT_HIDE_DROPDOWN : EVENT_SHOW_DROPDOWN);
		}
		*/
	};
	...
	p.addSuggestion = function(suggestion)
	{
		var self     = this,
			...
			node     = self.addDropdownItem(renderer ? renderer.call(self, suggestion) : self.itemManager().itemToString(suggestion.split('|')[0]))
			;
	...
	p.selectFromDropdown = function()
	{
		var self       = this,
			...
			;
		//console.log(self.selectedSuggestionElement().children().attr('valuetype'));
		if(suggestion)
		{
